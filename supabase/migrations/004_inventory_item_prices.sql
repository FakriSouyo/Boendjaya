-- Add manual current-price management and creation of inventory materials.
-- Current price recalculates menu COGS but does not rewrite past purchase records.
...
begin
  ...
  if p_current_stock < 0 or p_minimum_stock < 0 or p_average_cost < 0 then
    raise exception 'Nilai stok, batas restock, dan harga tidak boleh negatif';
  end if;
  ...
  if v_delta <> 0 then
    ...
    values (p_ingredient_id, 'adjustment', v_delta, p_average_cost, 'inventory_adjustment', coalesce(nullif(trim(p_note), ''), 'Penyesuaian stok fisik'));
  end if;
...
begin
  ...
  insert into public.ingredients (name, unit, current_stock, minimum_stock, average_cost)
  values (trim(p_name), p_unit, p_current_stock, p_minimum_stock, p_average_cost)
  returning id into v_ingredient_id;
  if p_current_stock > 0 then
    insert into public.stock_movements (ingredient_id, type, quantity, unit_cost, reference_type, note)
    values (v_ingredient_id, 'adjustment', p_current_stock, p_average_cost, 'initial_inventory', coalesce(nullif(trim(p_note), ''), 'Stok awal bahan baru'));
  end if;
